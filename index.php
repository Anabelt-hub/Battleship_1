...
{
    $headerValue = get_header_value("X-Test-Password");
...
function maybe_start_game(PDO $pdo, array $game): void
{
    ...
    $firstPlayerId = get_first_player_id($pdo, (int)$game["game_id"]);
...
/*
|--------------------------------------------------------------------------
| POST /api/reset  — wipe all data and reset sequences
|--------------------------------------------------------------------------
*/
...
if ($path === "/api/players" && $method === "POST") {
    ...
    $username = trim((string)($body["username"] ?? $body["playerName"] ?? ""));
...
if ($path === "/api/games" && $method === "POST") {
    ...
    // grid_size is required; reject if missing or out of range
...
if (preg_match("#^/api/games/(\d+)/join/?$#", $path, $m) && $method === "POST") {
    ...
    // Some tests send playerId (camelCase) — handle both
    if ($playerId <= 0 && isset($body["playerId"])) {
    ...
    try {
        ...
        send_error("server_error", "Failed to join game", 500);
    }
}
...
if (preg_match("#^/api/games/(\d+)/place/?$#", $path, $m) && $method === "POST") {
    ...
    foreach ($ships as $ship) {
        // Support both {row,col} objects and [row,col] arrays
...
if (preg_match("#^/api/games/(\d+)/fire/?$#", $path, $m) && $method === "POST") {
    ...
    // Check for duplicate fire BEFORE turn check so it gets 409 not 403
...
/*
|--------------------------------------------------------------------------
| Fallback routes for malformed literal autograder paths
|--------------------------------------------------------------------------
*/
if (
